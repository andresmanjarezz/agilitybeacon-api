...
def test_get_agilityPlans(client, test_agilityPlan, superuser_token_headers):
    ...
    response = client.get("/api/v1/agilityPlans", headers=superuser_token_headers)
...
def test_delete_agilityPlan(client, test_agilityPlan, test_db, superuser_token_headers):
    response = client.delete(
        f"/api/v1/agilityPlans/{test_agilityPlan.id}", headers=superuser_token_headers
    )
    assert response.status_code == 200
...
def test_edit_agilityPlan(client, test_agilityPlan, superuser_token_headers):
    ...
    assert response.status_code == 200
    assert all(
        response.json()[arg] == update_agilityPlan[arg] for arg in update_agilityPlan
    )
